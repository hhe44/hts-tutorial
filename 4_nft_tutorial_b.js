...
async function main() {
	...
	console.log(`============================ PART 2A BEGIN ============================\n`);

	// Enable Token KYC for Bob & Alice
	let aliceKyc = await kycEnabledFnt(aliceId);
	let bobKyc = await kycEnabledFnt(bobId);
	...
	// Query to check init. KYC key
	let initTokenInfo = await tQueryFnt();
	console.log(`- KYC key for the NFT is: \n${initTokenInfo.kycKey.toString()}\n`);

	// Update Token KYC key
	let tokenUpdateTx = await new TokenUpdateTransaction()
		.setTokenId(tokenId)
		.setKycKey(newKycKey)
		.freezeWith(client)
		.sign(adminKey); // updates require an admin signature if token was created w/ it
	let tokenUpdateSubmitTx = await tokenUpdateTx.execute(client);
	let tokenUpdateRx = await tokenUpdateSubmitTx.getReceipt(client);
	console.log(`- Token update on KYC key: ${tokenUpdateRx.status}\n`);

	// Query to check updated KYC key
	let finalTokenInfo = await tQueryFnt();
	console.log(`- KYC key for the NFT is: \n${finalTokenInfo.kycKey.toString()}\n`);

	console.log(`============================ PART 2A END ============================\n`);
	...
	console.log(`============================ PART 2B BEGIN ============================\n`);

	// Schedule transaction for NFT transfer from Alice to Bob - requires both signatures
	...
	let scheduleTxId = scheduleRx.scheduledTransactionId;
	console.log(`- Schedule Id is ${scheduleId} with Transaction Id ${scheduleTxId}\n`);

	// Get Alice's signature first
	...
	console.log(`- Alice's Signature Submission is: ${aliceSignRx.status}`);

	// check if schedule was triggered (should be no, still waiting on Bob)
	let initScheduleQuery = await new ScheduleInfoQuery().setScheduleId(scheduleId).execute(client);
	console.log(`- Has schedule triggered? ${initScheduleQuery.executed !== null}\n`);
	...
	console.log(`- Bob's Signature Submission is: ${bobSignRx.status}`);

	// check if schedule was triggered (should be no, still waiting on Bob)
	let finalScheduleQuery = await new ScheduleInfoQuery().setScheduleId(scheduleId).execute(client);
	console.log(`- Has schedule triggered? ${finalScheduleQuery.executed !== null}\n`);
	...
	// KYC enabling function
	async function kycEnabledFnt(id) {
		...
		let kyxRx = await kycSubmitTx.getReceipt(client);
		return kyxRx;
	}

	// token query function
	async function tQueryFnt() {
